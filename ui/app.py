...
from ui.interactive_pipeline import solve_for_thrust, solve_for_thrust_and_MR, ThrustSolveError
from ui.flight_sim import setup_flight, detect_tank_underfill_time
...
from ui.design_optimization_view import design_optimization_view
from copv.copv_solve_both import (
...
from engine.pipeline.thermal.graphite_geometry import size_graphite_insert
from engine.pipeline.thermal.graphite_cooling import compute_graphite_recession, calculate_throat_recession_multiplier
...
def copv_view(config_obj: PintleEngineConfig) -> None:
    ...
    with st.form("copv_sizing_form"):
        ...
        default_z_table = Path(__file__).parent.parent / "copv" / "n2_Z_lookup.csv"
...
def timeseries_view(runner: PintleEngineRunner, config_label: str) -> None:
    ...
    if mode == "Generate profile":
        ...
        if submitted:
            ...
            # Recession animation (if ablative cooling enabled)
            if (hasattr(runner.config, 'ablative_cooling') and 
                runner.config.ablative_cooling and 
                runner.config.ablative_cooling.enabled and
                runner.config.ablative_cooling.track_geometry_evolution):
                st.markdown("### 🎬 Recession Animation")
                if st.checkbox("Generate Recession Animation", value=False, key="recession_anim_gen"):
                    try:
                        from engine.pipeline.animation_fixed import create_working_animation
                        import traceback
...
# NOTE: flight_sim_view function is defined below (after graphite_insert_view)
# to ensure it has access to all required helper functions and the ROCKETPY check
...
def _display_chamber_results(results: dict, config_obj=None) -> None:
    ...
    if config_obj is None:
        ...
        if config_obj is None and config_dict:
            ...
            try:
                from engine.pipeline.io import load_config_from_dict
    ...
    # CRITICAL: Show layers even if only some components are enabled (not all required)
    # This will display ablative, graphite, and stainless steel layers when available
    if (calculate_complete_chamber_geometry is not None and 
        ...
        try:
            ...
            from engine.pipeline.chamber_geometry_visualizer import calculate_chamber_geometry_clear, plot_chamber_geometry_clear
    ...
    if calculate_button:
        try:
            ...
            if "graphite_timeseries" in st.session_state and input_method == "From Engine Run":
                from engine.pipeline.thermal.graphite_geometry import verify_transient_thickness
...
def flight_sim_view(runner: PintleEngineRunner, config_obj: PintleEngineConfig, config_label: str) -> None:
    ...
    with st.expander("Rocket", expanded=False):
        st.caption("**Mass Model:** Airframe (body/fins/avionics) + Propulsion (engine + tanks) + Propellant (LOX + Fuel)")
        rocket = working.get("rocket") if working.get("rocket") is not None else {}
        
        # New mass model defaults
        ...
        _airframe = rocket.get("airframe_mass")
        airframe = st.number_input(
            "Airframe mass [kg]", 
            value=float(_airframe) if _airframe is not None else 78.72, 
        ...
        with colE1:
            _engine_mass = rocket.get("engine_mass")
            engine_mass = st.number_input(
                "Engine + plumbing [kg]", 
                value=float(_engine_mass) if _engine_mass is not None else 8.0, 
                key="flight_engine_mass",
                help="Chamber, nozzle, injector, valves, ALL fittings & plumbing"
            )
        with colE2:
            _lox_tank_mass = rocket.get("lox_tank_structure_mass")
            lox_tank_mass = st.number_input(
                "LOX tank [kg]", 
                value=float(_lox_tank_mass) if _lox_tank_mass is not None else 5.0, 
                key="flight_lox_tank_mass",
                help="Empty LOX tank only (walls, no fittings)"
            )
        with colE3:
            _fuel_tank_mass = rocket.get("fuel_tank_structure_mass")
            fuel_tank_mass = st.number_input(
                "Fuel tank [kg]", 
                value=float(_fuel_tank_mass) if _fuel_tank_mass is not None else 3.0, 
                key="flight_fuel_tank_mass",
        ...
        if auto_inertia:
            ...
            _inertia = rocket.get("inertia")
            if _inertia is None or len(_inertia) < 3:
        ...
        lox_tank_temp = working.get("lox_tank", {})
        fuel_tank_temp = working.get("fuel_tank", {})
        ox_pos = float(lox_tank_temp.get("ox_tank_pos", 0.6))
        fuel_pos = float(fuel_tank_temp.get("fuel_tank_pos", -0.2))
    ...
    with st.expander("Tanks", expanded=False):
        ...
        # Get fluid densities for capacity validation
        fluids_temp = working.get("fluids") if working.get("fluids") is not None else {}
        ox_temp = fluids_temp.get("oxidizer") if fluids_temp.get("oxidizer") is not None else {}
        fu_temp = fluids_temp.get("fuel") if fluids_temp.get("fuel") is not None else {}
        rho_lox = float(ox_temp.get("density", 1140.0))
        rho_fuel = float(fu_temp.get("density", 780.0))
        ...
        st.caption(f"Tank Volume: {lox_volume*1000:.1f} L | Max Capacity: {lox_capacity:.1f} kg | Fill: {lox_fill_pct:.0f}%")
        if lox_mass_input > lox_capacity * 0.98:
            st.warning(f"⚠️ LOX mass ({lox_mass_input:.1f} kg) exceeds 98% tank capacity ({lox_capacity:.1f} kg)!")
        ...
        st.caption(f"Tank Volume: {fuel_volume*1000:.1f} L | Max Capacity: {fuel_capacity:.1f} kg | Fill: {fuel_fill_pct:.0f}%")
    ...
    with st.expander("Nozzle (visualization only)", expanded=False):
        st.info("ℹ️ **Visualization only** — These values are used to draw the nozzle in the rocket diagram. "
                "They do NOT affect trajectory simulation (thrust curve already includes nozzle effects).")
        
        nozzle = working.get("nozzle") if working.get("nozzle") is not None else {}
        nozzle.setdefault("A_exit", 0.00831498636119)
        nozzle.setdefault("efficiency", 0.98)  # Kept for config compatibility
        
        # Only A_exit is used (for nozzle_radius in visualization)
        nozzle["A_exit"] = float(st.number_input(
            "Exit area [m²]", 
            value=float(nozzle["A_exit"]), 
            format="%.8f", 
            key="flight_noz_ae",
            help="Used to draw the nozzle exit diameter in visualization"
        ))
        
        # Show nozzle exit diameter for reference
        D_exit = np.sqrt(4 * nozzle["A_exit"] / np.pi) * 1000  # mm
        st.caption(f"Exit Ø: {D_exit:.1f} mm (used for rocket drawing)")
        
        # Set A_throat from config or default (not editable - not used)
        nozzle.setdefault("A_throat", 0.00156235266901)
        if nozzle["A_throat"] > 0:
            nozzle["expansion_ratio"] = nozzle["A_exit"] / nozzle["A_throat"]
        else:
            nozzle["expansion_ratio"] = 1.0
...
def main():
    ...
    tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8, tab9, tab10 = st.tabs([
        "🎯 Design Optimization",  # NEW: Comprehensive optimization workflow
        "Forward Mode",
        "Inverse Mode",
